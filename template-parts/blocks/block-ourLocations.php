...
<section id="<?php echo esc_attr($id); ?>" class="<?php echo esc_attr($className); ?>">
    ...
    <div class="buttons">
        <?php 
            ...
            foreach ( $terms as $term ) {
                ...
                $sermon_series_date = get_field( 'taxonomy_order', $term );
                $sermon_series[$sermon_series_date] = $term;
            }
            krsort( $sermon_series, SORT_NUMERIC );
            if ( ! empty( $terms ) && ! is_wp_error( $terms ) ){
                echo '<button type="button" data-filter="all">'. pll__('ALL HARBOURS') .'</button>';
    ...
    <div class="containerr">
        ...
        global $post;
            ...
            $get_terms = get_the_terms( $post->ID, 'location_cat' );
            $boat_size = get_field( 'location_size', get_the_id() );
        ?>
        <div class="mix <?php foreach ( $get_terms as $term ) {echo "category-" . $term->term_id;}?>">
            <a href="<?php the_permalink(); ?>">
                <?php 
                $image = get_field('location_thumbnail',get_the_id());
                if( !empty( $image ) ): ?>
                    <figure>
